...
class ContactController {
    ...
    def save(Contact contact) {
        ...
        try {
            ...
            if(!multipartFile.empty) {
                contact.properties = [filename:multipartFile.getOriginalFilename(), data:[bytes:multipartFile.getBytes()]]
            }
    ...
    */
    ...
    def c1() {
        ...
        def instanceList = Contact.where {
    ...
    def listDirsForJsTree() {
        ...
        Contact.findAllByType("文件夹").each { dir->
            def hm = new HashMap()
    ...
    def moveTo(String id, String pid) {
        ...
        def instance = Contact.get(id)
        if(pid.equalsIgnoreCase("0")) {
            instance.parent = null
        } else {
            instance.parent = Contact.load(pid)
        }

        try {
            contactService.save(instance)
        } catch (ValidationException e) {
            render status: INTERNAL_SERVER_ERROR
            return
        }
